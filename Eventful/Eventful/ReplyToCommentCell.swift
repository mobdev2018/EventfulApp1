//
//  ReplyToCommentCell.swift
//  Eventful
//
//  Created by Dad's Gift on 02/02/18.
...
protocol ReplyToCommentCellDelegate: class {
    func optionsButtonTapped(cell: ReplyToCommentCell)
    func handleProfileTransition(tapGesture: UITapGestureRecognizer)
}
class ReplyToCommentCell: UICollectionViewCell {
    weak var delegate: ReplyToCommentCellDelegate? = nil
    ...
    var didTapOptionsButtonForCell: ((ReplyToCommentCell) -> Void)?
    
    var comment: ReplyToComment?{
        didSet{
    ...
    override init(frame: CGRect){
        ...
        profileImageView.anchor(top: topAnchor, left: leftAnchor, bottom: nil, right: nil, paddingTop: 8, paddingLeft: 50, paddingBottom: 0, paddingRight: 0, width: 40, height: 40)
